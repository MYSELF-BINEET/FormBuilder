...
export const signUp = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    sendEmail({
      email: newUser.email,
      subject: 'Welcome to IForm.Com',
      message: 'Thank you for signing up with IForm.Com!',
    });
...
export const forgotPassword = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const message =
      'You are receiving this email because you have just requested to reset your IForm.Com password. Please click on the link below or copy and paste the URL in a new browser window to reset your password:\n\n' +
      `${resetUrl}\n\n` +
    ...
    try {
      await sendEmail({
        email: foundUser.email,
        subject:
          'Password reset token for IForm.Com account (valid for 10 minutes)',
        message,
      });
